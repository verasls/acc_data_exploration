accuracyIndices <- function(df, predicted.var, dependent.var) {
  ...
  # Absolute values
  bias.mean <- round(mean(df[ , predicted.var] - df[ , dependent.var]), digits = 2)
  bias.sd   <- round(sd(df[ , predicted.var] - df[ , dependent.var]), digits = 2)
  LoA.inf   <- round(bias.mean - (1.96 * bias.sd), digits = 2)
  LoA.sup   <- round(bias.mean + (1.96 * bias.sd), digits = 2)
  MAE       <- round(mean(abs(df[ , predicted.var] - df[ , dependent.var])), digits = 2)
  MAE.sd    <- round(sd(abs(df[ , predicted.var] - df[ , dependent.var])), digits = 2)
  RMSE      <- round(sqrt(mean((df[ , predicted.var] - df[ , dependent.var])^2)), digits = 2)
  
  # Percent values
  bias.mean.p <- round(mean(((df[ , predicted.var] - df[ , dependent.var]) / df[ , predicted.var])) * 100, digits = 2)
  bias.sd.p   <- round(sd(((df[ , predicted.var] - df[ , dependent.var]) / df[ , predicted.var])) * 100, digits = 2)
  LoA.inf.p   <- round(bias.mean.p - (1.96 * bias.sd.p), digits = 2)
  LoA.sup.p   <- round(bias.mean.p + (1.96 * bias.sd.p), digits = 2)
  MAPE        <- round(mean(abs((df[ , predicted.var] - df[ , dependent.var]) / df[ , predicted.var])) * 100, digits = 2)
  MAPE.sd     <- round(sd(abs((df[ , predicted.var] - df[ , dependent.var]) / df[ , predicted.var])) * 100, digits = 2)
